...
LangtonAntDIDI::LangtonAntDIDI(unsigned int x, unsigned int y, unsigned int dir, unsigned int xlim, unsigned int ylim):
            Ant(x, y, dir, xlim, ylim){}

LangtonAntDIDI::~LangtonAntDIDI(){}

Color LangtonAntDIDI::move(Color colorDeCuadrado){    //Up 0, Down 1, Lft 2, Rght 3       //0 BLCK 1 WHITE 2 BLUE 3 RED
    //assert(direction_ >= 0 && direction_ < 4);
    if(colorDeCuadrado == Blanco){
       //Blanco ' ', mueve 90º a la izquierda           L
        switch (direction_) {
        case 0: //Up goes left
            direction_ = 2;
            if((yPos_ - 1) < 1){
...
void LangtonAntDIDI::paint() const{
    switch (direction_) {
    case 0:     //UP
        std::cout << " ^ " ;
        break;
